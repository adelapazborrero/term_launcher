...
- `ls` at root tags each folder line with `folderName` (`TerminalEntry.folder()`), rendering a 📁 icon + name (brackets stripped) instead of plain `[name]` text. Terminal mode keeps the original plain-text listing.
